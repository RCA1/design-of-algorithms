/***
 *     
 *     PROGRAM #2 BY: RAYVIN CARL AMADOR
 *
 *     FALL 2017 - CSC2400 DESIGN OF ALGORITHMS
 *     PROFESSOR:DR. DAVID BROWN
 *     DUE DATE: FRIDAY December 1, 2017
 *
 *     In this assignment, you will be finding the minimum spanning tree of a graph in two ways.
 *     -- Prim's Algorithm discussed starting on page 318 of your text book
 *     -- Kruskal's Algorithm discussed starting on page 325 of your text book
 *          For Kruskal's use the quick union method as discussed in class.
 *     -- You will also implement Dijkstra's shortest path algorithm using the efficient technique (using decreaseKey) discussed in class.
 *     
...
void Graph<T>::addVertex(T item) // ITEM IS A POINTER TO A CITY OBJECT 
{
	...
	cout << left;
	cout << "After adding " << setw(4) << "--- " << setw(15) << city_name << 
	"  Array Size: " << setw(3) << current_size_of_array << setw(3) << " " << 
